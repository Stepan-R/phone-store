...
export const ProductInfo = () => {
  ...
  useEffect(() => {
    setCurrentPicture(currentPhone?.images[0]);
  }, [currentPhone]);
  ...
  function getUpdatedCapacity(capacity: string) {
    if (phones.find(phone => phone.id === currentPhone?.id)) {
      ...
      const updatedPhoneCapacity = sameNameSpaceId.find(
        phone =>
          phone.capacity === capacity && phone.color === currentPhone?.color,
      );

      return updatedPhoneCapacity?.id;
    }
    ...
    if (tablets.find(tablet => tablet.id === currentPhone?.id)) {
      ...
      const updatedTabletCapacity = sameNameSpaceId.find(
        tablet =>
          tablet.capacity === capacity && tablet.color === currentPhone?.color,
      );

      return updatedTabletCapacity?.id;
    }
    ...
    if (accessories.find(accessorie => accessorie.id === currentPhone?.id)) {
      ...
      const updatedAssecCapacity = sameNameSpaceId.find(
        accessorie =>
          accessorie.capacity === capacity &&
          accessorie.color === currentPhone?.color,
      );

      return updatedAssecCapacity?.id;
    }
  ...
  function getUpdatedPhoneColor(color: string) {
    if (phones.find(phone => phone.id === currentPhone?.id)) {
      const sameNameSpaceId = phones.filter(
        phone => phone.namespaceId === currentPhone?.namespaceId,
      );
    ...
    if (tablets.find(tablet => tablet.id === currentPhone?.id)) {
      ...
      const updatedTabletColor = sameCapacity.find(
        tablet => tablet.color === color,
      );

      return updatedTabletColor?.id;
    }
    ...
    if (accessories.find(accessorie => accessorie.id === currentPhone?.id)) {
      ...
      const updatedAccessorieColor = sameCapacity.find(
        accessorie => accessorie.color === color,
      );

      return updatedAccessorieColor?.id;
    }
  ...
  useEffect(() => {
    if (currentPhone) {
      const newPath = `/${currentPhone.category}/${currentPhone.id}`;

      if (newPath !== location.pathname) {
        navigate(newPath);
      }
    }
  }, [currentPhone, navigate, location.pathname]);
              ...
              <div className="product-colors-block">
                ...
                <div className="btns-block">
                  {currentPhone?.colorsAvailable.map(color => (
                    <Link
                      to={`/${currentPhone?.category}/${getUpdatedPhoneColor(color)}`}
                      className={
              ...
              <div className="gb-container">
                ...
                <div className="gb-btns-block">
                  {currentPhone?.capacityAvailable.map(capacity => (
                    <Link
                      to={`/${currentPhone?.category}/${getUpdatedCapacity(capacity)}`}
                      className={
